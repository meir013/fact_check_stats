# -*- coding: utf-8 -*-
"""
Created on Mon Jan 20 14:07:55 2020
...
cols=['author','claimReviewed','title','datePublished','dateCreated','itemReviewed','appear','url']
...
for x in df['dataFeedElement']:
    ...
    try:
        ...
        if 'itemReviewed' in item and 'appearance' in item['itemReviewed']:
            urls=[]
            for a in item['itemReviewed']['appearance']:
                if "facebook" not in a["url"] and "youtube" not in a["url"]:
                    urls.append(a["url"])
            if len(urls)>0:
                dic['appear']=urls
            else:
...
data.to_csv('claimreview_jan20.csv', mode='a', header=False)

#%% 
#data['itemReviewed'].value_counts().head(50)
data["appear"].values.tolist()
#%% 
import itertools
list(itertools.chain(*[x for x in data["appear"].values.tolist() if x is not None]))
#%% 
#import datetime
